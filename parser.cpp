...
#define did_not_find_next(x)//  next_token(); if(!have(x)) did_not_find(x);
#define did_not_find_fail(x) if(!have(x)) did_not_find(x)
...
static ast parse_class()
{
    ...
    TokenKind tk = token_kind(t);

    string class_name;
    ast statics = create_empty();
    ast subrs = create_empty();
    ast fields = create_empty();
        ...
        while(tk != tk_eoi)
        {
            switch(tk)
            {
                case tk_class:
                    {
                        next_token();
                        did_not_find_fail(tk_identifier);

                        class_name = token_spelling();

                        did_not_find_next(tk_lcb);
                    }
                    break;
                case tk_static:
                    {
    ...
    ast ret = create_class(class_name, statics, fields, subrs);
    pop_error_context() ;
    return ret ;
}
...
static ast parse_static_var_decs()
{
    ...
    vector<ast> statics;
    statics.push_back(parse_static_var_dec());

    pop_error_context() ;
    return create_class_var_decs(statics);
}
...
static ast parse_static_var_dec()
{
    ...
    // add code here ...
    did_not_find_next(tk_identifier);

    string var_type = token_spelling(next_token());
    string var_name = token_spelling(next_token());
    vector<ast> statics;
    int offset = 0;

    statics.push_back(create_var_dec(var_name, "static", offset, var_type));
    while(have_next(tk_comma))
    {
        did_not_find_fail(tk_identifier);
        var_name = token_spelling(next_token());
        // TODO : declare var
        statics.push_back(create_var_dec(var_name, "static", offset, var_type));
        offset++;
        next_token();
    ...
    ast ret = create_var_decs(statics);
...
static ast parse_field_var_decs()
{
    ...
    vector<ast> fields;
    fields.push_back(parse_field_var_dec());

    pop_error_context() ;
    return create_class_var_decs(fields);
}
...
static ast parse_field_var_dec()
{
    ...
    fields.push_back(create_var_dec(var_name, "this", offset, var_type));
    while(have_next(tk_comma))
    {
        did_not_find_fail(tk_identifier);
        var_name = token_spelling(next_token());
    ...
    ast ret = create_var_decs(fields);
